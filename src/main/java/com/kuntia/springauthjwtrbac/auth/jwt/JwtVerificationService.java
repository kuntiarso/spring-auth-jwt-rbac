...
public class JwtVerificationService implements JwtService {

    @Value("${JWT_REFRESH_SECRET}")
    private String jwtVerificationSecret;

    @Value("${JWT_REFRESH_EXPIRES_IN}")
    private long jwtVerificationExpiresIn;

    @Override
    public Date extractExpiration(String token) {
        return JwtUtils.extractClaim(token, Claims::getExpiration, jwtVerificationSecret);
    }

    @Override
    public String extractUsername(String token) {
        return JwtUtils.extractClaim(token, Claims::getSubject, jwtVerificationSecret);
    }
